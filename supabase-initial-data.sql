-- QuoteMaster Pro - Initial Data Setup
...
SELECT 'free', 'Free Plan', 0.00, 'monthly', '{"max_quotes": 5, "max_storage_mb": 100, "support": "community"}'
WHERE NOT EXISTS (SELECT 1 FROM subscription_plans WHERE name = 'free');

INSERT INTO subscription_plans (name, description, price, billing_interval, features)
SELECT 'pro', 'Pro Plan', 29.99, 'monthly', '{"max_quotes": 100, "max_storage_mb": 1000, "support": "email", "custom_branding": true}'
WHERE NOT EXISTS (SELECT 1 FROM subscription_plans WHERE name = 'pro');

INSERT INTO subscription_plans (name, description, price, billing_interval, features)
SELECT 'enterprise', 'Enterprise Plan', 99.99, 'monthly', '{"max_quotes": -1, "max_storage_mb": 10000, "support": "priority", "custom_branding": true, "api_access": true}'
...
SELECT 'app_name', '"QuoteMaster Pro"', 'Application name', 'general', true
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'app_name');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'app_version', '"1.0.0"', 'Application version', 'general', true
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'app_version');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'default_currency', '"USD"', 'Default currency for quotes and invoices', 'financial', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'default_currency');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'max_file_size_mb', '10', 'Maximum file upload size in MB', 'uploads', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'max_file_size_mb');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'email_from_address', '"noreply@quotemaster.pro"', 'Default from email address', 'email', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'email_from_address');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'stripe_publishable_key', '""', 'Stripe publishable key', 'payments', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'stripe_publishable_key');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'stripe_secret_key', '""', 'Stripe secret key', 'payments', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'stripe_secret_key');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'stripe_webhook_secret', '""', 'Stripe webhook secret', 'payments', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'stripe_webhook_secret');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'paypal_client_id', '""', 'PayPal client ID', 'payments', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'paypal_client_id');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'paypal_client_secret', '""', 'PayPal client secret', 'payments', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'paypal_client_secret');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'enable_registrations', 'true', 'Allow new user registrations', 'auth', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'enable_registrations');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'require_email_verification', 'false', 'Require email verification for new users', 'auth', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'require_email_verification');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'maintenance_mode', 'false', 'Enable maintenance mode', 'system', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'maintenance_mode');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'max_users', '-1', 'Maximum number of users (-1 for unlimited)', 'limits', false
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE key = 'max_users');

INSERT INTO system_settings (key, value, description, category, is_public)
SELECT 'default_quote_validity_days', '30', 'Default quote validity period in days', 'quotes', false
...
SELECT 'quote_sent', 'New Quote: {{quote_number}}',
 ...
 '["quote_number", "client_name", "total_amount", "valid_until", "business_name"]'
...
SELECT 'invoice_created', 'Invoice: {{invoice_number}}',
 ...
 '["invoice_number", "client_name", "amount", "due_date", "business_name"]'
...
INSERT INTO email_templates (name, subject, html_content, text_content, variables)
SELECT 'payment_received', 'Payment Received: {{invoice_number}}',
 '<h1>Payment Received</h1><p>Dear {{client_name}},</p><p>We have received your payment for invoice {{invoice_number}}.</p><p>Amount Paid: {{amount_paid}}</p><p>Payment Date: {{payment_date}}</p><p>Thank you for your business!</p><p>Best regards,<br>{{business_name}}</p>',
 'Dear {{client_name}}, We have received your payment for invoice {{invoice_number}}. Amount Paid: {{amount_paid}}. Payment Date: {{payment_date}}. Thank you for your business! Best regards, {{business_name}}',
 '["client_name", "invoice_number", "amount_paid", "payment_date", "business_name"]'
WHERE NOT EXISTS (SELECT 1 FROM email_templates WHERE name = 'payment_received');
